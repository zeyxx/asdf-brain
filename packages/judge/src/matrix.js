 ...
 *
 * @module @asdf/cynic-judge/matrix
...
const { PHI, PHI_2, PHI_INV, PHI_INV_2 } = require('../../core/src/axioms/constants');
...
/**
 * Default paths - can be overridden via configure()
 */
let MATRIX_DIR = path.join(__dirname, '../../../knowledge/cynic/matrices');
let WEIGHTS_FILE = path.join(MATRIX_DIR, 'weights.json');
let HARMONY_FILE = path.join(MATRIX_DIR, 'harmony.json');
let THRESHOLDS_FILE = path.join(MATRIX_DIR, 'thresholds.json');

/**
 * Configure paths for different environments
 * @param {Object} config - { matrixDir }
 */
function configure(config = {}) {
  if (config.matrixDir) {
    MATRIX_DIR = config.matrixDir;
    WEIGHTS_FILE = path.join(MATRIX_DIR, 'weights.json');
    HARMONY_FILE = path.join(MATRIX_DIR, 'harmony.json');
    THRESHOLDS_FILE = path.join(MATRIX_DIR, 'thresholds.json');
  }
}

/**
 * Get current paths (for testing/debugging)
 */
function getPaths() {
  return {
    MATRIX_DIR,
