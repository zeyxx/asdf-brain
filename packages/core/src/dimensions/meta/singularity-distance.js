...
class SingularityDistanceEvaluator extends DimensionEvaluator {
  constructor() {
    ...
    // Distance levels (φ-scaled) - using instance PHI values from base class
    this.DISTANCES = {
      SINGULARITY: 0,              // Unknowable absolute truth
      ATZILUT: this.PHI_INV,       // 0.618 - Primordial concepts
      BERIAH: 1,                   // 1.0 - Created understanding
      YETZIRAH: this.PHI,          // 1.618 - Formed knowledge
      ASSIAH: this.PHI * this.PHI, // 2.618 - Manifested action
    };
  }
  ...
  _calculateDistance(score) {
    ...
    if (score < 40) return { world: 'ATZILUT', value: this.PHI_INV };
    if (score < 60) return { world: 'BERIAH', value: 1 };
    if (score < 80) return { world: 'YETZIRAH', value: this.PHI };
    return { world: 'ASSIAH', value: this.PHI * this.PHI };
  }
