...
const { loadAccessTracking } = require('./_shared');
 ...
 * Merges with access tracking data for accurate N-Score calculation
 */
async function loadKnowledgeNodes() {
  try {
    ...
    // Load access tracking data (from brain_search usage)
    const accessTracking = loadAccessTracking();

    return lines.map((line, idx) => {
      try {
        const entry = JSON.parse(line);
        const nodeId = entry.id || `node_${idx}`;

        // Merge access tracking data if available
        const tracked = accessTracking[nodeId] || {};
        ...
        return {
          ...
          // Map to N-Score fields - prefer tracked data over entry defaults
          accesses: tracked.accesses || entry._accesses || 1,
          lastAccessed: tracked.lastAccessed || entry._lastAccessed || new Date(entry.timestamp).getTime(),
