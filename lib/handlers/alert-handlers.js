...
async function handleAlertCheck(args, adapter) {
  try {
    ...
    // Include git intelligence data for git alert rules
    let gitData = null;
    try {
      const gitScan = gitIntelligence.scanEcosystem({ judge: true });
      gitData = {
        alertable: gitScan.alertable || [],
        summary: gitScan.summary,
        cynicEnabled: gitScan.cynicEnabled,
      };
    } catch (e) {
      // Git intelligence errors don't block alert checking
      gitData = { error: e.message, alertable: [] };
    }

    // Merge git data into diagnostic for alert rule checking
    const enrichedDiagnostic = {
      ...diagnostic,
      git: gitData,
    };

    const results = await alerts.checkAll(enrichedDiagnostic);
    ...
    return {
      ...
      gitProblems: gitData.alertable?.length || 0,
