...
class ContextInjector {
  ...
  /**
   * Get relevant philosophy principles
   */
  async _getPhilosophyAnchor(query) {
    const principles = [
      { key: 'verify', text: "Don't trust, verify", trigger: /verify|trust|signature|check/i },
      { key: 'burn', text: "100% burn - no extraction", trigger: /burn|fee|treasury/i },
      { key: 'phi', text: "φ guides all ratios", trigger: /ratio|percent|distribution|weight/i },
      { key: 'align', text: "Perfect alignment of incentives", trigger: /incentive|align|everyone/i },
      { key: 'open', text: "Open source as moat", trigger: /open|fork|license|mit/i },
    ];
    ...
    for (const p of principles) {
      if (p.trigger.test(queryText)) {
        relevant.push(p);
      }
    }

    // Always include verify as base principle
    if (relevant.length === 0) {
      relevant.push(principles[0]);
    }

    return { principles: relevant };
  }
