...
// DataAdapter for caching (lazy-loaded to avoid circular deps)
let _dataAdapter = null;
function getAdapter() {
  if (!_dataAdapter) {
    try {
      const { getDataAdapter } = require('./cynic/data-adapter');
      _dataAdapter = getDataAdapter({ basePath: path.join(__dirname, '../knowledge') });
    } catch (e) {
      _dataAdapter = null;
    }
  }
...
class ContextStore {
  ...
  _loadPersisted() {
    // Load persisted sessions (active sessions survive restarts)
    const sessionsPath = path.join(this.basePath, 'sessions.json');
    if (fs.existsSync(sessionsPath)) {
      try {
        const sessions = JSON.parse(fs.readFileSync(sessionsPath, 'utf-8'));
        const now = new Date();
        for (const session of sessions) {
          // Only load active sessions less than 2 hours old
          const startedAt = new Date(session.started_at);
          const hoursOld = (now - startedAt) / (1000 * 60 * 60);
          if (session.active && hoursOld < 2) {
            this.sessions.set(session.id, session);
          }
        }
      } catch (e) {
        // Silent fail - corrupted file
      }
    }
...
class SessionManager {
  ...
  startSession(options = {}) {
    ...
    this.store._persist(); // Persist immediately so other processes can see it
...
class ContextInjector {
  ...
  /**
   * Load patterns specific to a project (with caching)
   */
  async _loadProjectPatterns(projectId) {
    try {
      const adapter = getAdapter();

      // Use cached read if adapter available
      let data;
      if (adapter) {
        data = await adapter.readJSON('patterns/extracted-patterns.json');
      } else {
  ...
  /**
   * Load recent intent for a project (with caching)
   */
...
class ContextLayer {
  ...
  // Reload sessions from disk (for cross-process sync)
  reloadSessions() {
    const sessionsPath = path.join(this.store.basePath, 'sessions.json');
    if (fs.existsSync(sessionsPath)) {
      try {
        const sessions = JSON.parse(fs.readFileSync(sessionsPath, 'utf-8'));
        const now = new Date();
  ...
  getStats() {
    // Reload from disk to get cross-process sessions
    this.reloadSessions();
