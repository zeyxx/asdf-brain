...
class ContextInjector {
  ...
  async _getPhilosophyAnchor(query) {
    const axioms = [
      {
        ...
        trigger: /phi|φ|1\.618|0\.618|ratio|percent|weight|threshold|geometric|golden/i,
        derivations: ['K-Score', 'E-Score', 'I-Infra', 'CYNIC limits', 'PaRDeS weights'],
      },
      {
        key: 'burn',
        text: '100% BURN - all value converges to singularity',
        trigger: /burn|fee|treasury|deflat|supply|alignment|incentive/i,
      ...
      },
      {
        ...
        trigger: /verify|trust|signature|hmac|merkle|proof|integrity|tamper|sign/i,
      ...
      },
      {
        ...
        trigger: /culture|moat|open.*source|mit|license|fork|cypherpunk|community|marketing/i,
    ...
    for (const a of axioms) {
      if (a.trigger.test(queryText)) {
        relevant.push(a);
      }
    }

    // Always include verify as base axiom (core principle)
    if (relevant.length === 0) {
      relevant.push(axioms[2]); // verify
    }

    return { axioms: relevant, _note: '4 axioms, everything else derives' };
  }
