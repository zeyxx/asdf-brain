...
function registerDefaultSubsystems() {
  // Observation subsystem - CYNIC's continuous observation
  registerSubsystem('observation', async () => {
    try {
      const { getResidualDetector } = require('./judge');
      const { getInnommable } = require('./innommable');

      const detector = getResidualDetector();
      const innommable = getInnommable();

      const residualStats = detector.getStats();
      const innommableStatus = innommable.getStatus();

      // Check if we should trigger pattern discovery
      if (residualStats.bufferStats.shouldCluster) {
        const discovery = detector.discoverDimensions();

        // Route discovered patterns to THE_INNOMMABLE
      ...
      return {
        ...
        details: {
          residuals: {
            count: residualStats.bufferStats.count,
            weightedCount: Math.round(residualStats.bufferStats.weightedCount * 100) / 100,
            shouldCluster: residualStats.bufferStats.shouldCluster,
          },
          innommable: {
            pending: innommableStatus.pendingCount,
            accepted: innommableStatus.stats.accepted,
            rejected: innommableStatus.stats.rejected,
          },
          status: 'observing',
        },
      };
      ...
      return {
        healthy: true, // Non-critical - observation failing shouldn't alert
        details: { error: error.message, status: 'degraded' },
      };
    }
  });

  // Dashboard subsystem - Updates dimensions dashboard on every pulse
  registerSubsystem('dashboard', async () => {
    try {
      const { saveDashboard } = require('./dashboard-dimensions');
      const { getDataAdapter } = require('./data-adapter');

      const adapter = getDataAdapter();
      const dashboardPath = path.join(__dirname, '../../knowledge/dashboard/dimensions.html');

      await saveDashboard(dashboardPath);

      return {
        healthy: true,
        details: {
          path: dashboardPath,
          cache: adapter.getStats().cache,
      ...
      return {
        ...
        details: { error: error.message, status: 'refresh_failed' },
      };
    }
  });
