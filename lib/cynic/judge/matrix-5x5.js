  ...
  FOUNDATION: {
    ...
    dimensions: {
      ...
      EVIDENCE_BASE: {
        ...
        maps_to: 'TRUTH',  // Was INTEGRITY (cryptographic), TRUTH = "Is it verifiable?"
        axiom: 'VERIFY'
      },
      LOGICAL_COHERENCE: {
  ...
  META: {
    ...
    dimensions: {
      ...
      MEASURABILITY: {
        ...
        maps_to: 'SELF_AWARENESS',  // Was INTEGRITY, SELF_AWARENESS = "Je sais ce que je ne sais pas"
        axiom: 'META'  // META axiom since it's about meta-cognition
      },
      LEARNABILITY: {
...
const VERDICTS = {
  // φ-based thresholds (max = 61.8%)
  // HOWL requires: high confidence AND no dimensions <50%
  HOWL: { threshold: 0.60, emoji: '🐕', description: 'Exceptional quality, rare achievement' },  // Very close to φ⁻¹
  WAG: { threshold: 0.52, emoji: '🐕', description: 'Generally good, minor improvements possible' },  // Above average
  GROWL: { threshold: 0.382, emoji: '🐕', description: 'Minor issues, address before proceeding' },  // φ⁻²
...
function mapFrom4Worlds(judgment) {
  ...
  const dimensionReasons = judgment.reasons || {};

  for (const [rowName, row] of Object.entries(MATRIX_5x5)) {
    for (const [dimName, config] of Object.entries(row.dimensions)) {
      const sourceScore = dimensionScores[config.maps_to];
      const sourceReason = dimensionReasons[config.maps_to];
