...
const { ResidualDetector } = require('./residual-detector');

// =============================================================================
// RESIDUAL DETECTOR INSTANCE (Singleton for observation continuity)
// =============================================================================

let residualDetectorInstance = null;

/**
 * Get or create the residual detector instance
 * Singleton ensures residual collection persists across calls
 */
function getResidualDetector() {
  if (!residualDetectorInstance) {
    residualDetectorInstance = new ResidualDetector();
  }
...
function evaluateDimension(item, dimension, context = {}) {
  ...
  switch (dimension) {
    ...
    // DISCOVERED dimensions (φ⁻¹ weight) - discovered via residual analysis
    case 'CULTURAL_CONTEXT':
      baseScore = evaluateCulturalContext(item, context);
      reasoning.push('Checked cultural alignment');
      break;
...
function evaluateCulturalContext(item, context) {
  const content = item.content || '';
  // DISCOVERED dimension: Check cultural/tradition alignment
  // Based on CULTURE axiom: "Culture is a moat"
  const hasCulture = /culture|tradition|heritage|community|ritual|ceremony|moat/i.test(content);
  const hasHistory = /history|origin|legacy|ancestor|generation/i.test(content);
  const hasContext = /context|local|regional|native|indigenous/i.test(content);
  const hasAntiCulture = /destroy tradition|erase culture|uniform|homogen/i.test(content);

  if (hasAntiCulture) return 30;
  if (hasCulture && hasHistory) return 75;
  if (hasCulture || hasContext) return 68;
  if (hasHistory) return 60;
  return 52; // φ⁻¹ × 100 ≈ 61.8, but starts conservative for discovered dimension
}
...
function judge(input, options = {}) {
  ...
  // ==========================================================================
  // RESIDUAL DETECTION - Fermer le circuit vers DISCOVER
  // "φ qui découvre ce qu'il ne sait pas"
  // ==========================================================================
  try {
    const detector = getResidualDetector();

    // Prepare judgment in format expected by residual-detector
    const judgmentForResidual = {
      global: result.global,
      scores: result.dimensions,
      verdict: result.verdict,
      confidence: result.confidence,
      worlds: {}, // Could be extended with world-level scores
    };
    ...
    // Attach residual info to result
    result.residual = {
    ...
    if (residualResult.isAnomaly) {
      result.residual.bufferStatus = residualResult.bufferStatus;
      result.residual.clusterSuggestion = residualResult.clusterSuggestion;
    }
  } catch (err) {
