...
const { emergentManager } = require('../dimensions/emergent');
...
class ResidualConnector {
  ...
  attemptDiscovery() {
    ...
    if (result.discovered && result.candidates && result.candidates.length > 0) {
      for (const candidate of result.candidates) {
        this.stats.dimensionsProposed++;

        // Forward to EmergentDimensionManager
        const proposalResult = emergentManager.receiveProposal({
          suggestedName: candidate.suggestedName,
          suggestedWorld: candidate.axiomAlignment, // World maps to axiom in discovery
          suggestedAxiom: candidate.axiomAlignment,
          suggestedQuestion: `Does it satisfy ${candidate.suggestedName}?`,
          confidence: candidate.confidence,
          clusterSize: candidate.clusterSize,
          patterns: Object.values(candidate.features || {}).filter(v => typeof v === 'string'),
        ...
        this.cynic.emit('residual:proposal', {
          ...
          clusterSize: candidate.clusterSize,
          suggestedName: candidate.suggestedName,
          suggestedWorld: candidate.axiomAlignment,
          suggestedAxiom: candidate.axiomAlignment,
          forwarded: proposalResult.accepted,
        });
      }
    }
   ...
   */
  acceptProposal(proposalId, config = {}) {
    // Delegate to EmergentDimensionManager
    const result = emergentManager.validateProposal(proposalId, {
      name: config.name,
      world: config.world,
      axiom: config.axiom,
      threshold: config.threshold || 50,
      question: config.question,
      validator: config.validator || 'human',
    });
  ...
  rejectProposal(proposalId, reason = '') {
    return emergentManager.rejectProposal(proposalId, reason);
  }

  /**
   * Get pending proposals awaiting validation
   */
  getPendingProposals() {
    return emergentManager.getPendingProposals();
  }
...
function connect(cynic, options = {}) {
  ...
  return {
    ...
    accept: (proposalId, config) => connector.acceptProposal(proposalId, config),
    reject: (proposalId, reason) => connector.rejectProposal(proposalId, reason),
    getPending: () => connector.getPendingProposals(),
