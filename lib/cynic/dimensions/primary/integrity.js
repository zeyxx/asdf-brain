...
class IntegrityEvaluator extends DimensionEvaluator {
  ...
  _evaluateStructuralIntegrity(obs) {
    let score = 40; // Baseline for any observation
    const reasons = [];
    ...
    } else if (presentFields.length >= 2) {
      score += 10;
    ...
    if (obs.parentId || obs.relatedTo || obs.references) {
      ...
      const hasValidRefs = (
        (obs.parentId && String(obs.parentId).length > 5) ||
    ...
    // Future creation date is suspicious
    if (obs.createdAt) {
      const created = new Date(obs.createdAt).getTime();
      if (created > Date.now() + 60000) { // More than 1 min in future
        contradictions++;
