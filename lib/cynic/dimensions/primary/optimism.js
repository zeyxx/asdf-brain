...
class OptimismEvaluator extends DimensionEvaluator {
  ...
  /**
   * Growth Mindset - Evaluates explicit trait FLAGS only (no text scanning to avoid overlap)
   * Text-based detection is handled by sentiment/constructive evaluators
  ...
  _evaluateGrowthMindset(obs) {
    ...
    // === RESILIENCE: explicit flags only ===
    ...
    // === ADAPTABILITY: explicit flags only ===
    ...
    const score = traits.reduce((sum, t) => sum + t, 0) / traits.length;
