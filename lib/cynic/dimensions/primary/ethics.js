...
class EthicsEvaluator extends DimensionEvaluator {
  ...
  async evaluate(observation, context = {}) {
    ...
    // 6. Cypherpunk Values Checklist - REUSES existing scores + adds SELF_CUSTODY & PERMISSIONLESS
    const valuesScore = this._evaluateCypherpunkValues(observation, details);
    scores.push({ value: valuesScore.score, weight: 0.8 });
    details.cypherpunkValues = valuesScore;
  ...
  /**
   * Cypherpunk Values Summary - REUSES existing scores, adds 2 new checks
   * Maps: privacy→PRIVACY, decentralization→DECENTRALIZATION, openness→OPEN_SOURCE, extraction→NO_EXTRACTION
   * Adds: SELF_CUSTODY, PERMISSIONLESS (not covered by other evaluators)
   */
  _evaluateCypherpunkValues(obs, details) {
    const reasons = [];

    // REUSE existing scores (no duplication)
    const valueScores = {
      PRIVACY: details.privacy?.score ?? 50,
      DECENTRALIZATION: details.decentralization?.score ?? 50,
      OPEN_SOURCE: details.openness?.score ?? 50,
      NO_EXTRACTION: details.extraction?.score ?? 50,
      // NEW evaluations (not covered elsewhere)
      SELF_CUSTODY: 50,
      PERMISSIONLESS: 50,
    };

    // Collect reasons from existing evaluators
    if (details.privacy?.reasons?.length) reasons.push(...details.privacy.reasons.slice(0, 1));
    if (details.decentralization?.reasons?.length) reasons.push(...details.decentralization.reasons.slice(0, 1));
    if (details.openness?.reasons?.length) reasons.push(...details.openness.reasons.slice(0, 1));
    if (details.extraction?.reasons?.length) reasons.push(...details.extraction.reasons.slice(0, 1));

    // === SELF-CUSTODY: "Not your keys, not your coins" (NEW) ===
    if (obs.selfCustody === true || obs.nonCustodial === true) {
      valueScores.SELF_CUSTODY += 35;
      reasons.push('self-custody ✓');
    }
    if (obs.userControlled === true || obs.ownsKeys === true) {
      valueScores.SELF_CUSTODY += 15;
    }
    if (obs.custodial === true || obs.thirdPartyCustody === true) {
      valueScores.SELF_CUSTODY -= 30;
      reasons.push('custodial ⚠️');
    }

    // === PERMISSIONLESS: "Ask forgiveness, not permission" (NEW) ===
    if (obs.permissionless === true || obs.censorship_resistant === true) {
      valueScores.PERMISSIONLESS += 30;
      reasons.push('permissionless ✓');
    }
    if (obs.trustless === true || obs.noKYC === true) {
      valueScores.PERMISSIONLESS += 20;
    }
    if (obs.kyc === true || obs.requiresApproval === true) {
      valueScores.PERMISSIONLESS -= 25;
      reasons.push('permissioned ⚠️');
    }

    // Clamp new scores
    valueScores.SELF_CUSTODY = Math.max(0, Math.min(100, valueScores.SELF_CUSTODY));
    valueScores.PERMISSIONLESS = Math.max(0, Math.min(100, valueScores.PERMISSIONLESS));

    // Calculate average
    const values = Object.values(valueScores);
    const score = values.reduce((sum, v) => sum + v, 0) / values.length;

    // Count aligned and violated
    const alignedCount = values.filter(v => v >= 60).length;
    const violatedCount = values.filter(v => v < 40).length;
  ...
  _buildReasoning(details, finalScore) {
    ...
    // Highlight cypherpunk values alignment
    if (details.cypherpunkValues?.summary) {
      const { aligned, violated, total } = details.cypherpunkValues.summary;
      if (aligned === total) {
        parts.push('6/6 cypherpunk values ✓');
      } else if (violated > 0) {
        parts.push(`${violated} value violation(s)`);
      } else {
        parts.push(`${aligned}/${total} values`);
      }
    }
