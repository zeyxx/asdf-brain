...
class EthicsEvaluator extends DimensionEvaluator {
  ...
  async evaluate(observation, context = {}) {
    ...
    // 6. Cypherpunk Values Checklist (20 points) - NEW: Explicit value tracking
    const valuesScore = this._evaluateCypherpunkValues(observation);
  ...
  /**
   * Evaluate alignment with explicit cypherpunk values
   * Core principles: Privacy, Decentralization, Open Source, Self-Custody, Permissionless, No Extraction
   */
  _evaluateCypherpunkValues(obs) {
    let score = 50;
    const reasons = [];
    const valueScores = {
      PRIVACY: 50,
    ...
    // === PRIVACY: "Privacy is not about something to hide" ===
    if (obs.privacy === true || obs.privacyPreserving === true) {
      valueScores.PRIVACY += 30;
      reasons.push('privacy ✓');
    }
    if (obs.encrypted === true || obs.zeroKnowledge === true) {
      valueScores.PRIVACY += 20;
    }
    if (obs.tracking === true || obs.surveillance === true) {
      valueScores.PRIVACY -= 30;
      reasons.push('surveillance ⚠️');
    }

    // === DECENTRALIZATION: "No single point of control" ===
    if (obs.decentralized === true || obs.distributed === true) {
      valueScores.DECENTRALIZATION += 30;
      reasons.push('decentralized ✓');
    }
    if (obs.p2p === true || obs.peer2peer === true) {
      valueScores.DECENTRALIZATION += 20;
    }
    if (obs.centralized === true || obs.singlePointOfFailure === true) {
      valueScores.DECENTRALIZATION -= 35;
      reasons.push('centralized ⚠️');
    }

    // === OPEN SOURCE: "Code is law, but law must be readable" ===
    if (obs.openSource === true || obs.opensource === true) {
      valueScores.OPEN_SOURCE += 35;
      reasons.push('open source ✓');
    }
    if (obs.auditable === true || obs.verifiable === true) {
      valueScores.OPEN_SOURCE += 15;
    }
    if (obs.closedSource === true || obs.proprietary === true) {
      valueScores.OPEN_SOURCE -= 35;
      reasons.push('proprietary ⚠️');
    }
    ...
    // === PERMISSIONLESS: "Ask forgiveness, not permission" ===
    ...
    // === NO EXTRACTION: "Don't extract, burn" ===
    if (obs.burn === true || obs.burnModel === true) {
      valueScores.NO_EXTRACTION += 35;
      reasons.push('burn-aligned 🔥');
    }
    if (obs.noFees === true || obs.communityOwned === true) {
      valueScores.NO_EXTRACTION += 15;
    }
    if (obs.vcFunded === true || obs.rentSeeking === true) {
      valueScores.NO_EXTRACTION -= 35;
      reasons.push('extractive ⚠️');
    }

    // Clamp all value scores
    for (const key of Object.keys(valueScores)) {
      valueScores[key] = Math.max(0, Math.min(100, valueScores[key]));
    }
    ...
    // Count aligned and violated values
