...
class SkillJudge {
  constructor(options = {}) {
    ...
    // Load evidence data for empirical validation
    this.evidence = this._loadEvidence();
  ...
  _loadEvidence() {
    try {
      ...
      this.logger.warn('[SkillJudge] Failed to load evidence:', e.message);
    }
    return this._createEmptyEvidence();
  }

  /**
   * Create empty evidence structure
   */
  _createEmptyEvidence() {
    return {
      version: 1,
      ...
      metrics: {
        totalValidated: 0,
        accuracyRate: null,
        brierScore: null,  // Proper scoring rule
        lastValidation: null
      }
    };
  }
  ...
  _saveEvidence() {
    try {
      ...
      this.evidence.updated = new Date().toISOString();
      fs.writeFileSync(EVIDENCE_PATH, JSON.stringify(this.evidence, null, 2));
    } catch (e) {
      this.logger.error('[SkillJudge] Failed to save evidence:', e.message);
    }
  }

  /**
   * Record validation outcome for evidence tracking
   * @param {Object} originalJudgment - The judgment being validated
   * @param {string} outcome - 'confirmed' | 'refuted' | 'partial'
   * @param {Object} corrections - Dimension corrections if any
   */
  _recordEvidence(originalJudgment, outcome, corrections = {}) {
    ...
    this.evidence.calibration.buckets[bucket].predictions++;
    if (outcome === 'confirmed') {
      this.evidence.calibration.buckets[bucket].correct++;
    }

    // 2. Update verdict accuracy
    if (this.evidence.verdicts[verdict]) {
      this.evidence.verdicts[verdict].total++;
      this.evidence.verdicts[verdict][outcome]++;
    }

    // 3. Update dimension-level evidence
    for (const [dimName, delta] of Object.entries(corrections)) {
      if (!this.evidence.dimensions[dimName]) {
        this.evidence.dimensions[dimName] = {
          validations: 0,
          totalError: 0,
          meanAbsoluteError: null
        };
      }
      const dimEvidence = this.evidence.dimensions[dimName];
    ...
    this.evidence.metrics.totalValidated++;
    this.evidence.metrics.lastValidation = new Date().toISOString();

    // Calculate accuracy rate
    const allVerdicts = Object.values(this.evidence.verdicts);
    ...
    this.evidence.metrics.accuracyRate = totalValidated > 0
      ? totalConfirmed / totalValidated
      : null;

    // Calculate Brier score (lower is better, 0 = perfect)
    // Brier = mean((forecast - outcome)²) where outcome is 1 for confirmed, 0 for refuted
    this._updateBrierScore(confidence, outcome);

    // Calculate Expected Calibration Error
    this._updateCalibrationError();
  ...
  /**
   * Update Brier score (proper scoring rule for probabilistic predictions)
   */
  _updateBrierScore(confidence, outcome) {
    ...
    const n = this.evidence.metrics.totalValidated;
    const currentBrier = this.evidence.metrics.brierScore || 0;
    this.evidence.metrics.brierScore = ((n - 1) * currentBrier + squaredError) / n;
  }

  /**
   * Calculate Expected Calibration Error (ECE)
   * ECE = sum(|bucket_accuracy - bucket_confidence| * bucket_weight)
   */
  _updateCalibrationError() {
    const buckets = this.evidence.calibration.buckets;
    let ece = 0;
    ...
    this.evidence.calibration.expectedCalibrationError = totalPredictions > 0
   ...
   * @param {Object} feedback.originalJudgment - The judgment being validated (for evidence tracking)
  ...
  learn(feedback = {}) {
    ...
    // EVIDENCE: Record validation outcome for empirical metrics
    if (originalJudgment) {
      this._recordEvidence(originalJudgment, outcome, corrections);
    }
  ...
  // ===========================================================================
  // EVIDENCE METHODS (EVIDENCE_BASE)
  // ===========================================================================

  /**
   * Get empirical evidence data
   *
   * Returns calibration metrics, verdict accuracy, and dimension-level evidence.
   * Use this to understand how well CYNIC's judgments match reality.
   */
  getEvidence() {
    ...
    evidence.summary = {
      ...
      reliability: this._computeReliability(evidence)
    };
  ...
  /**
   * Compute overall reliability score (0-1) from evidence
   */
  _computeReliability(evidence) {
    ...
    // Factor 3: Brier score (lower = better, convert to 0-1 score)
  ...
  /**
   * Get evidence summary as human-readable report
   */
  getEvidenceReport() {
    const e = this.getEvidence();
    const lines = [
      '=== CYNIC EVIDENCE REPORT ===',
      '',
      `Total Validations: ${e.metrics.totalValidated}`,
      `Accuracy Rate: ${e.metrics.accuracyRate !== null ? (e.metrics.accuracyRate * 100).toFixed(1) + '%' : 'N/A'}`,
      `Brier Score: ${e.metrics.brierScore !== null ? e.metrics.brierScore.toFixed(3) : 'N/A'} (lower is better)`,
      `Calibration Error: ${e.calibration.expectedCalibrationError !== null ? e.calibration.expectedCalibrationError.toFixed(3) : 'N/A'}`,
      `Reliability: ${e.summary.reliability !== null ? (e.summary.reliability * 100).toFixed(1) + '%' : 'Insufficient data'}`,
      '',
      '--- Verdict Breakdown ---'
    ];

    for (const [verdict, data] of Object.entries(e.verdicts)) {
      if (data.total > 0) {
    ...
    for (const [bucket, data] of Object.entries(e.calibration.buckets)) {
      if (data.predictions > 0) {
    ...
    if (Object.keys(e.dimensions).length > 0) {
      lines.push('', '--- Dimension Accuracy ---');
      const sortedDims = Object.entries(e.dimensions)
  ...
  /**
   * Reset evidence data (use with caution)
   */
  resetEvidence() {
    this.evidence = this._createEmptyEvidence();
    this._saveEvidence();
