...
const PHI_INV = 1 / PHI;                    // 0.618 - healthy threshold
const PHI_INV_2 = 1 / (PHI * PHI);          // 0.382 - warning threshold
...
async function checkHoldexHealth() {
  try {
    ...
    // Check rate limiter
    checks.rateLimiterOk = status.rate_limiter?.available > 0;
    ...
    return healthResult(
      ...
      score,
      {
        ...
        lastEvent: status.stats?.last_event_time || null,
        errorCount: status.stats?.error_count || 0,
        rateLimiterAvailable: status.rate_limiter?.available || 0,
        checks,
      }
    );
  } catch (error) {
