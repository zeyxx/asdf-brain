...
class SelfJudge {
  ...
  async _judgeIntent(item, context) {
    ...
    if (item.intent || context.intent) {
      score = 80;
      reason = 'Intent explicit';
    }

    // Goal defined?
    if (item.goal || context.goal) {
      score += 10;
    }
  ...
  async _judgeTrust(item, context) {
    ...
    let score = 50;
    let reason = 'Trust neutral';

    // Human trusts LLM?
    if (context.humanTrusts === true || item.humanTrusts === true) {
      score += 25;
    }

    // LLM trusts human input?
    if (context.llmTrusts === true || item.llmTrusts === true) {
      score += 25;
    }

    // Verification requested? (healthy skepticism)
    if (item.verificationRequested === true) {
      score += 10;
      reason = 'Healthy verification';
    }

    // Trust explicitly broken?
    if (item.trustBroken === true || context.trustBroken === true) {
      score = 20;
      reason = 'Trust broken';
    }

    return { score: Math.min(100, score), reason: reason || 'Trust established' };
  }
