...
/**
 * Quick judgment using CYNIC-JUDGE (with residual detection)
 *
 * Now routes through judge.js which includes:
 * - SelfJudge (4 Mondes)
 * - ResidualDetector (THE_INNOMMABLE connection)
 * - Full judgment pipeline
 *
 * @param {Object} item - Item to judge
 * @param {Object} context - Judgment context
 * @returns {Promise<Object>} Judgment result with residual info
 */
async function judge(item, context = {}) {
  // Use the full judge pipeline which includes residual detection
  return judgeModule.judge(item, context);
}
...
module.exports = {
  ...
  strategicAnalysis: vision.analyze,
  checkSystemHealth: vision.checkHealth,
  forecast: vision.forecast,
  getVisionSummary: vision.getSummary,
  recordResidual: discover.recordResidual,
  analyzeResiduals: discover.analyzeResiduals,
  proposeNewDimension: discover.proposeNewDimension,
  validateDimension: discover.validateDimension,
  integrateDimension: discover.integrateDimension,
  discover: discover.discover,
  getDiscoverySummary: discover.getSummary,
  getMetaDimension: discover.getMetaDimension,
  digest: digest.digest,
  digestQuick: digest.digestQuick,
  digestRoadmap: digest.digestRoadmap,
  extractIdeas: digest.extractIdeas,
