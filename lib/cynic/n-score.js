...
function calculate(node, context = {}) {
  ...
  // Defaults - use 1 as minimum to avoid penalizing low-access nodes
  // When scoring multiple nodes, these are calculated from actual data
  const maxAccesses = context.maxAccesses || 1;
  const maxEdges = context.maxEdges || 1;
  ...
  // Age factor: starts at φ⁻² (38.2%) and grows to 100% over time
  // This gives new knowledge "benefit of the doubt" instead of starting at 0
  // Formula: φ⁻² + (1 - φ⁻²) × (1 - e^(-t/τ))
  const daysSinceCreation = (now - createdAt) / (1000 * 60 * 60 * 24);
  const rawAgeFactor = 1 - Math.exp(-daysSinceCreation / DECAY.TRUST_MATURITY);
  const ageFactor = PHI_INV_2 + (1 - PHI_INV_2) * rawAgeFactor;
