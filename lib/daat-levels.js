...
const { CYNIC, SelfJudge } = require('./cynic');
...
function createDaatCynic(level = 1) {
    // SelfJudge already has all 16 dimensions loaded
    // Daat level affects judgment depth, not which dimensions exist
    //
    // Level 1 (PASSIVE): Quick check, no full judgment
    // Level 2 (SUGGESTIVE): Standard judgment
    // Level 3 (ACTIVE): Thorough judgment with scaling
    // Level 4 (STRATEGIC): Full judgment with refinement

    if (level < 2) {
        // Level 1: No CYNIC evaluation, just pass-through
        return new CYNIC({ evaluator: null });
    }

    // Levels 2-4: Use full SelfJudge with all dimensions
    const judge = new SelfJudge();

    return new CYNIC({
        evaluator: judge,
        // Store level for judgment mode selection
        _daatLevel: level
    });
}
